...
Moonbeam's cross-chain integration is accomplished by becoming a [parachain](/ecosystem/platform/glossary/#parachains) on the Polkadot network.  The [Polkadot network](/ecosystem/platform/glossary/#polkadot) provides integration and connectivity between parachains that are connected to the network and to other non-Polkadot-based chains, such as Ethereum and Bitcoin, via bridges.
...
Ecosystem projects that need smart contract functionality can use Moonbeam to:  
 ...
 - Add new functionality that is needed but not included on the main [Polkadot relay chain](/ecosystem/platform/glossary/#relay-chain). For example, they could create a place where teams can crowdfund their projects, implement lockdrops, and process other, more complex financial transactions than are provided by base [Substrate](/ecosystem/platform/glossary/#substrate) functionality.  
